...
void store_info_in_header(const std::string &info_type, const std::string &info_content, std::map<std::string, std::string> &header) {
    std::map<std::string, std::string> original_header = header; // Store the original header

    if (info_type == "lang") {
        header["Accept-Language"] = info_content; // Store language info
    } else if (info_type == "encoding") {
        header["Accept-Encoding"] = info_content; // Store encoding info
    } else if (info_type == "cache") {
        header["Cache-Control"] = info_content; // Store cache info
    } else {
        return; // Invalid info_type, return original header (no change)
    }
}
