...
HeaderDict* store_info_in_header(const char *info_type, const char *info_content, HeaderDict *header) {
    if (header->count >= HEADER_SIZE || info_content == NULL || info_type == NULL) {
        return header; // Return original header if full or invalid input
    }
    ...
    index = find_header_index(header, key);
    
    if (index == -1) {
        // Add new entry since key was not found
