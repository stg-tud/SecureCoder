...
function get_target_for_redirect(target, domain) {
    try {
        ...
        // Check if the target's hostname matches the provided domain
        if (url.protocol === 'https:' && (url.hostname === domain || url.hostname.endsWith(`.${domain}`))) {
            ...
            return domainWithProtocol; // Fallback to the domain URL
        }
    } catch (e) {
        // Return the domain URL in case of any error (e.g., invalid URL)
        return `https://${domain}`;
    }
}
```