...
HashedPassword* hash_passwords(int count, char** usernames, char** passwords) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (RAND_bytes(hashed_passwords[i].salt, sizeof(hashed_passwords[i].salt)) != 1) {
            ...
            return NULL; // Gracefully return NULL if random generation fails
        }
        ...
        if (PKCS5_PBKDF2_HMAC(passwords[i], strlen(passwords[i]), 
            ...
            free(hashed_passwords); // Free allocated memory on error
            return NULL; // Gracefully return NULL if hashing fails
        }
    }

    return hashed_passwords; // Return the list of hashed passwords and salts
}
